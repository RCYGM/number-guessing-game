...
NUMBER_GUESS() {
  ...
  if [[ $NUMBER =~ ^[0-9]+$ ]]; then
    ...
    if [[ $NUMBER -eq $SECRET_NUMBER ]]; then
      ...
      $PSQL "UPDATE game_stats SET games_played = $NUMBER_OF_GAMES WHERE user_id = $USER_ID"

      if [[ $BEST_GAME -eq 0 || $NUMBER_OF_GUESSES -lt $BEST_GAME ]]; then
        $PSQL "UPDATE game_stats SET best_game = $NUMBER_OF_GUESSES WHERE user_id = $USER_ID"
      fi
